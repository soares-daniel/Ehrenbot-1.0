...
class ChannelManager(commands.Cog):
    ...
    def __init__(self, bot) -> None:
        ...
        temp_channels = self.bot.database["temp_channels"].find_one({"channel_type": "voice_channels"})
        if temp_channels:
    ...
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        ...
        if after.channel:
            if after.channel.name.lower().startswith("New".lower()):
                if after.channel.id not in self.temp_channels:
                    ...
                    temp_channel = await after.channel.clone(name=f"{member.display_name}'s {channel_name}")
        ...
        self.bot.database["temp_channels"].update_one({"channel_type": "voice_channels"}, {"$set": {"channels": self.temp_channels}}, upsert=True)
