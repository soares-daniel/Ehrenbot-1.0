...
class Misc(commands.Cog):
    ...
    @commands.slash_command(
        name="violin", description="Plays the worlds smallest violin"
    )
    async def violin(self, ctx: discord.ApplicationContext):
        self.logger.info("%s used the violin command", ctx.author)
        if ctx.author.voice is None:
            await ctx.respond(
                "You are not in a voice channel", ephemeral=True, delete_after=5
            )
        else:
            try:
                voice = await ctx.author.voice.channel.connect()
                await ctx.respond(
                    "https://tenor.com/view/sad-upset-violin-sponge-bob-mr-crab-gif-3466351",
                    ephemeral=True,
                    delete_after=10,
                )
                ...
                await ctx.respond(
                    "Something went wrong", ephemeral=True, delete_after=5
                )
            finally:
    ...
    @commands.slash_command(
        name="select_character", description="Change your character symbol."
    )
