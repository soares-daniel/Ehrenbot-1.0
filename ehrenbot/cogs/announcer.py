...
class Announcer(commands.Cog):
    ...
    announce = discord.SlashCommandGroup(
        name="announce", description="Announcer commands."
    )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.channel.name == "server-announcements-sedam":
            channel = discord.utils.get(
                message.guild.channels, name="server-announcements"
            )
        if not channel:
            return
    ...
    async def on_message_edit(self, before: discord.Message, after: discord.Message):
        ...
        if before.channel.name == "ehrenbot-updates-sedam":
            channel: discord.TextChannel = discord.utils.get(
                before.guild.channels, name="ehrenbot-updates"
            )
        if before.channel.name == "server-announcements-sedam":
            channel: discord.TextChannel = discord.utils.get(
                before.guild.channels, name="server-announcements"
            )
        if not channel:
            return
