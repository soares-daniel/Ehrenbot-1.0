            ...
            for ticket in ticket_collection.find():
                ...
                if ticket_status != "Closed":
                    if ticket["ticket"]["category"] == "Clan Join Request":
                        self.bot.add_view(
                            ClanRequestView(bot=self.bot, logger=self.logger),
                            message_id=admin_message_id,
                        )
                    else:
                        self.bot.add_view(
                            view=TicketAdminView(bot=self.bot, logger=self.logger),
                            message_id=admin_message_id,
                        )
                        self.bot.add_view(
                            view=TicketUserView(bot=self.bot, logger=self.logger),
                            message_id=user_message_id,
                        )

    @commands.slash_command(
        name="ticket_system", description="Initializes the ticket system"
    )
...
class TicketSelect(discord.ui.View):
    ...
    options = [
        discord.SelectOption(
            label="Clan Join Request",
            emoji="👋",
            description="Request to join the clan",
            value="Clan Join Request",
        ),
        discord.SelectOption(
            label="Clan Support",
            emoji="👥",
            description="Request support for the clan",
            value="Clan Support",
        ),
        discord.SelectOption(
            label="User Report",
        ...
        discord.SelectOption(
            label="Suggestion",
            emoji="📝",
            description="Request a suggestion",
            value="Suggestion",
        ),
        discord.SelectOption(
            label="Other", emoji="❓", description="Other request", value="Other"
        ),
    ]
    ...
    async def select_callback(
        ...
        if value == "Clan Join Request":
            ...
            embed = discord.Embed(
                title="Clan Join Request", color=discord.Color.purple()
            )
            ...
            if tokens.count_documents({"discord_id": interaction.user.id}) == 0:
                await interaction.user.send(
                    "You have not registered your Bungie account yet. "
                    "Please use the `/register` command to do so."
                )
        else:
            await interaction.message.edit(content="", view=self)
            await interaction.response.send_modal(
                TicketModal(value, self.bot, self.logger)
            )
        self.logger.info(
            "User %s created a ticket with the category %s",
...
class TicketModal(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await admin_channel.send(
            embed=ticket_embed, view=TicketAdminView(self.bot, self.logger)
        )
        last_admin_message_id = admin_channel.last_message_id
        await interaction.user.send(
            embed=ticket_embed, view=TicketUserView(self.bot, self.logger)
        )
        ...
        ticket_collection.insert_one(
            {
                "ticket_id": ticket_id,
                "ticket": ticket,
                "admin_message_id": last_admin_message_id,
                "user_message_Id": last_user_message_id,
                "discord_id": interaction.user.id,
            }
        )
        await interaction.followup.send(
            "Ticket was created. Visit your DM's to see your ticket.",
            ephemeral=True,
            delete_after=5,
        )
...
class TicketUserView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Edit", style=discord.ButtonStyle.blurple, custom_id="edit"
    )
    async def edit(
        ...
        if not ticket:
            await interaction.response.send_message(
                "Ticket not found", ephemeral=True, delete_after=5
            )
            self.logger.error("Ticket %d not found", ticket_id)
            return
        await interaction.response.send_modal(
            TicketEditModal(self.bot, self.logger, embed)
        )
    ...
    async def close(
        ...
        embed = interaction.message.embeds[0]
        await interaction.response.send_modal(
            TicketCloseModal(self.bot, self.logger, embed)
        )
...
class TicketAdminView(discord.ui.View):
    ...
    @discord.ui.button(
        label="In Work", style=discord.ButtonStyle.green, custom_id="in_work"
    )
    async def in_work(
        self, button: discord.ui.Button, interaction: discord.Interaction
    ) -> None:
        ...
        if embed.color == discord.Color.gold():
            ...
            button.label = "Open"
            self.logger.info(
                "User %s set Ticket %d to In Work", interaction.user, ticket_id
            )
...
class TicketEditModal(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.followup.send(
            f"Ticket {ticket_id} was edited.", ephemeral=True, delete_after=5
        )
...
class ClanRequestView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Invite", style=discord.ButtonStyle.blurple, custom_id="invite"
    )
    async def invite(
        self, button: discord.ui.Button, interaction: discord.Interaction
    ) -> None:
        ...
        if admin_profile is None:
            await interaction.followup.send(
                "Admin profile not found, please register.",
                ephemeral=True,
                delete_after=5,
            )
        ...
        if user_profile is None:
            await interaction.followup.send(
                "Destiny profile not found", ephemeral=True, delete_after=5
            )
            self.logger.warn(
                "Destiny profile for %d not found. Notifying user to register profile.",
                discord_id,
            )
            user = self.bot.get_user(discord_id)
            await user.send(
                "Your profile was not found in the database. Please register your profile with the command `/registration_bungie`"
            )
            return
        ...
        admin_token: dict = token_collection.find_one(
            {"discord_id": interaction.user.id}
        )["token"]
        ...
        if response is None:
            await interaction.followup.send(
                "Invite failed", ephemeral=True, delete_after=5
            )
            self.logger.warn("Invite failed for %d", discord_id)
            return
        if response["ErrorCode"] == 1:
            await interaction.followup.send(
                "User was invited", ephemeral=True, delete_after=5
            )
            # Get member
            ...
            channel: discord.TextChannel = discord.utils.get(
                interaction.guild.channels, name="📮｜admin-tickets"
            )
            message = await channel.fetch_message(message_id)
            await message.delete()
        elif response["ErrorCode"] == 676:
            await interaction.followup.send(
                "User is already in the clan", ephemeral=True, delete_after=5
            )
            ...
            await interaction.followup.send(
                "You lack the privileges to invite someone to the clan!",
                ephemeral=True,
                delete_after=5,
            )
        else:
            await interaction.followup.send(
                "Something went wrong while inviting, check the logs.",
                ephemeral=True,
                delete_after=5,
            )
            self.logger.error(
                "Something went wrong while inviting user %d to Code Ehre\n%s",
                discord_id,
                response,
            )
